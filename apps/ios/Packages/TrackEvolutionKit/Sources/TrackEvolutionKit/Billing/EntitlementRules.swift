...
public extension Entitlement {
    ...
    static let FREE_CHANNELS = ["speed", "throttle", "brake"]
    static func canViewChannel(_ entitlement: Entitlement?, _ key: String) -> Bool {
        FREE_CHANNELS.contains(key) || isPro(entitlement)
    }

    /// The Pro half of the channel panel: every channel beyond `FREE_CHANNELS`
    /// (steering, RPM, lateral G, yaw, gear, the limit marks, the Car tab), the lap
    /// delta chart, sector splits and the two-lap compare. The server strips the
    /// channels themselves (rule 4); the delta and the sectors derive from the free
    /// speed trace, so for those this predicate is the gate, and it is also what
    /// decides whether the panel carries the locked note.
