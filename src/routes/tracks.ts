...
// `opted_in` is the viewer's own flag so the UI can offer the opt-in without
// a second request.
tracks.get("/tracks/:id/leaderboard", async (c) => {
  ...
  // "Fixing" the discrepancy reopens that hole. The bare `d` column rides
  // along with MIN(ms): SQLite's documented min/max behavior picks it from
  // the winning row.
