...
// Signs `payload` as an ES256 JWS with the given PKCS#8 P-256 key. Shared by
// the Sign in with Apple client secret below and the App Store Server API
// token in lib/billing/apple.ts — same shape, different key and claims.
export async function signES256(
  ...
  const signingInput = base64UrlJson({ alg: "ES256", ...header }) + "." + base64UrlJson(payload);
...
export async function appleClientSecret(opts: {
  ...
  return signES256(
    { kid: opts.keyId },
    { iss: opts.teamId, iat, exp: iat + 5 * 60, aud: APPLE_ISSUER, sub: opts.clientId },
    opts.privateKeyPem
  );
}
