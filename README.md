...
On any event page — or on the **New event** form, where the same import review
stages its sessions until *Create event* (see *Adding laps with the event* below)
— **Import video / telemetry…** turns recordings into sessions
...
## Adding laps with the event

The **New event** form on every client ends with an **Add laps** section, so
logging a day is one screen rather than a form and then a page. It offers two of
the event page's three doors — the video/telemetry import and hand entry; the
live recorder stays on the event page and the dashboard, since a recording is
not something to stage — and nothing is posted until the event exists: the form
holds the sessions, then `POST /events` and one `POST /events/:id/sessions` per
session, staged imports first in the order they were staged, then the hand-typed
one (dropped when no lap parses). That rule is `sessionsToCreate` in
`public/js/event-form.js`, ported as `EventFormSessions` to the iOS Kit and
Android `:core` and pinned by `contracts/logic/event-form.json`. The created
event's id is remembered by the form, so a session the server refuses leaves the
form up with the message and the next submit retries only the sessions still
pending, never creating the event twice. Offline, both requests queue like any
other logbook write and the session follows the event's temp id to its real one
on replay. On the web the import review renders inline under the form; on the
phones the importer opens as it does from an event page, its review's event
picker replaced by "the event you're creating", and its *Add to new event*
hands the drafts back to the form (`AppRouter.stagedSessions` on iOS,
`RecordingFlow.staged` on Android), which lists them, removable, until Create.
Editing an existing event has no such section — its page has the real card.
...
- **Units** are a per-user choice (account menu → Settings → Units, stored in
  ...
  **imperial** (mph, °F, psi, gallons, miles) or **metric** (km/h, °C, bar,
  ...
  time and stays that way. The iOS and Android apps render the preference too
  (Settings → Units, beside the theme), from the same account value: each has
