...
- `db.ts` — data-access helpers. All take `(db, userId, ...)` explicitly so ownership scoping is visible at call sites: `ownedEvent`/`ownedSession` guard nested writes, `resolveTrack` find-or-creates tracks by (name, config) (case-insensitive — name/config are `COLLATE NOCASE` columns, so `UNIQUE(user_id, name, config)` also rejects case-variant duplicates at the schema level; config is part of track identity so bests/goals never mix across layouts), and `EVENT_SELECT`/`listEvents`/`tracksSummary` produce event rows with lap aggregates. Tracks stay per-user, but each row carries a nullable `catalog_id` into the seeded `track_catalog` table — a canonical identity matched by name (NOCASE) via `catalogIdForName` on create and rename, so the same physical track is recognizable across users without sharing user-entered data. The catalog seed mirrors `US_TRACKS` in `public/js/us-tracks.js`; adding a track means a new migration plus the same name appended to that list.
