...
- `js/` — extracted modules, importable by unit tests: `format.js` (`fmtMs`/`parseTime`/`esc`…), `chart.js` (hand-rolled SVG `lineChart` + `multiLineChart` lap overlay; lower lap time plotted lower = improvement trends downward), `channel-graphs.js` (per-lap speed/RPM/lateral-G overlay charts on a shared driven-distance axis; up to 3 laps highlighted via chips in the `--chart-line`/`-b`/`-c` slot colors, the rest a dim envelope), `lap-stats.js` (per-session analysis: best-N avg, pace slope, warmup), `year-review.js` (year-in-review aggregation), `api.js` (fetch wrapper throwing `ApiError` with status), `theme.js`, `us-tracks.js`.
