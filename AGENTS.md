...
npm run contracts:generate  # captures API responses into contracts/golden/ (NS-03)
npm run fixtures:generate   # writes contracts/fixtures/ — web-produced artifacts the native ports read
```
...
**Native rewrite** (`apps/`, in progress — specs in `docs/specs/native/`, start at its `README.md`): first-party SwiftUI (iOS) and Jetpack Compose (Android) apps that will replace the Capacitor shells in `mobile/`. The backend (`src/`) and the web app (`public/`) do not change for it. `apps/android` is a separate Gradle build whose `:core` module is **pure Kotlin/JVM** — models, API client, recorder core, lap geometry, offline queue — so its tests run in seconds with no emulator or Android SDK (`cd apps/android && ./gradlew :core:test`); a Gradle check fails the build if an `android`/`androidx` dependency ever reaches it. Ported logic keeps the *same function names and constants* as its JS original so the two can be diffed by eye (`RecorderCore.kt` ↔ `public/js/record/core.js`, `JsMath.round` ↔ the `Math.round(v * f) / f` idiom), and each port carries its JS test cases across — **change one suite and you change the other**, or the clients have silently diverged. Cross-client wire formats are pinned by real artifacts, not prose: `contracts/golden/` for API responses, `contracts/fixtures/` for what the web client writes (see README). The telemetry file parsers are deliberately never ported — file import stays a web feature.
