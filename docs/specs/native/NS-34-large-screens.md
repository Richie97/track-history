...
| Web-only stays web-only | The two-event compare overlay, the setup notebook and its diff and year in review remain on the deferred list. A large screen makes them tempting and does not reopen the decision; that is done in the README's deferred section, not in a layout ticket — which is how `.vbo` import left the list (2026-09, for Porsche Track Precision exports on the phone), and it is the only way anything should. |
...
| No recording on the Mac — and there, **hidden** | The same iPad build runs on Apple silicon Macs as a *Designed for iPad* app (epic #230), and a Mac has Wi-Fi location and no GPS. Where the iPad merely leaves the route unworked, the Mac closes every door: `Platform.runsOnMac` (beside `LayoutClass`, not inside it — a 13-inch iPad and a Mac window are both expanded, and width cannot tell them apart) hides the dashboard button and the event card's recorder, and `AppRouter` resolves `.record` to its event. |
